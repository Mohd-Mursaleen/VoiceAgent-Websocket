    ...
    async def initialize(self):
        ...
        async with self.initialization_lock:
            ...
            for _ in range(self.min_connections):
                try:
                    ...
                    if client.connected:
                        ...
                        logger.warning(
                            "Failed to establish connection for a client during initialization"
                        )
            ...
            if successful_connections > 0:
                ...
                logger.info(
                    f"Successfully pre-initialized {successful_connections} OpenAI clients"
                )
            else:
    ...
    async def health_check(self):
        ...
        for client in self.available_clients:
            if client.connected:
                ...
                try:
                    logger.warning(
                        "Reconnecting disconnected client during health check"
                    )
                    await client.connect()
                    if client.connected:
                        healthy_clients.append(client)
        ...
        needed_clients = max(
            ...
            - len(self.available_clients)
            - len(self.in_use_clients),
        )
        if needed_clients > 0:
            logger.info(
                f"Creating {needed_clients} additional clients to maintain minimum pool size"
            )
            for _ in range(needed_clients):
                try:
                    ...
                    logger.error(
                        f"Failed to create additional client during health check: {e}"
                    )

        logger.info(
            f"Health check complete. Pool status: {len(self.available_clients)} available, {len(self.in_use_clients)} in use"
        )
