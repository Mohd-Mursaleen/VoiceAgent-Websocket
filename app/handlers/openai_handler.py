...
class OpenAIMessageHandler:
    ...
    async def process_response(self, response: dict):
        ...
        try:
            ...
            if response_type == "session.created" or response_type == "session.updated":
                ...
                logger.info(
                    f"Function call started: {response.get('function_call', {}).get('name', 'unknown')}"
                )
            elif response_type == "response.function_call.arguments.delta":
                logger.debug(
                    f"Function call arguments delta: {response.get('delta', {})}"
                )
    ...
    async def _handle_audio_delta(self, response: dict):
        ...
        try:
            ...
            if self._audio_chunk_count % 50 == 0:
                logger.info(
                    f"Processed {self._audio_chunk_count} audio chunks, estimated duration: {self.audio_duration_ms:.0f}ms"
                )
    ...
    async def _handle_interruption(self):
        ...
        try:
            ...
            if actual_audio_duration < 400:  # Less than 500ms, too short to truncate
                logger.debug(
                    f"Skipping interruption, audio too short: {actual_audio_duration:.0f}ms"
                )
                return
            ...
            logger.info(
                f"Interrupting assistant speech at {safe_truncate_time:.0f}ms (actual duration: {actual_audio_duration:.0f}ms)"
            )
    ...
    async def _handle_function_call(self, response: dict):
        ...
        try:
            ...
            logger.info(
                f"Function call from realtime API - Name: {function_name}, ID: {call_id}"
            )
            ...
            if call_id and function_name:
                try:
                    ...
                    result = {"status": "success", "result": function_result}

                    logger.info(
                        f"Function {function_name} executed with result: {function_result}"
                    )

                except Exception as func_error:
                    logger.error(
                        f"Error executing function {function_name}: {func_error}"
                    )
                ...
                logger.warning(
                    f"Missing call_id or function_name in function call response"
                )
            ...
            if "call_id" in locals() and call_id:
                await send_tool_result(
                    self.openai_client, call_id, {"status": "error", "error": str(e)}
                )
